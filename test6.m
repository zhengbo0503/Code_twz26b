...
mm = 500*ones(1,length(kkappa));
nn = 400*ones(1,length(kkappa));
epsln = eps('double')/2;
mmode = 3:5;
M = 15;
...
for i = 1:2
    ...
    for j = 1:length(kkappa)
        ...
        [Uref, Sref] = get_reference(A, 'l');
        [err1, relgap1] = compute_error(A, U1, 'l', Uref, Sref);
        ...
        [err2, ~ ] = compute_error(A, U2, 'l', Uref, Sref);
        ...
        [err3, ~ ] = compute_error(A, U3, 'l', Uref, Sref);
        err_dgejsv(j,i) = max(err3);

        % MATLAB
        [U4,S4,V4] = svd(A,'econ');
        [err4, ~] = compute_error(A, U4, 'l', Uref, Sref);
